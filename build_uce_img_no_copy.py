...
# TODO - Change cmd line opts to args
...
def copy_save_file(source, dest):
    try:
...
def call_mksquashfs(data_dir, target_file, exclude_string):
    cmd = 'mksquashfs "{0}" "{1}" -b 262144 -root-owned -nopad -e {2}'.format(data_dir, target_file, exclude_string)
    print('************ ', cmd)
...
def run(input_dir, output_file):
    root_work_dir = os.path.join(input_dir, 'buildtemp')
    safe_make_dir(root_work_dir)
    cart_temp_file, cart_save_file, md5string_hex_file = set_image_file_paths(root_work_dir)
    set_755(os.path.join(input_dir, 'exec.sh'))
    copy_save_file(os.path.join(input_dir, 'save.zip'), root_work_dir)
    call_mksquashfs(input_dir, cart_temp_file, 'save.zip buildtemp')

    # TEST OUTPUT
    sq_img_file_md5 = get_md5(cart_temp_file)
    print('SqashFS Image Initial:')
    print('md5: ', sq_img_file_md5)
    print('Size: ', os.path.getsize(cart_temp_file))
    # TEST END
    ...
    # TEST OUTPUT
    print('SqashFS Image After Padding:')
    print('md5: ', sq_img_file_md5)
    print('Size: ', os.path.getsize(cart_temp_file))
    # TEST END

    append_to_file(cart_temp_file, bytearray.fromhex(sq_img_file_md5))
    ...
    # TEST OUTPUT
    print('Save Image:')
    print('md5: ', save_part_md5)
    print('Size: ', os.path.getsize(cart_save_file))
    # TEST END

    append_to_file(cart_temp_file, bytearray.fromhex(save_part_md5))

    # TEST OUTPUT
    print('After 32B:')
    print('md5: ', cart_temp_file)
    print('Size: ', os.path.getsize(cart_temp_file))
    # TEST END


    append_file_to_file(cart_temp_file, cart_save_file)
...
if __name__ == "__main__":
    ...
    run(options.input_dir, options.out_file)
